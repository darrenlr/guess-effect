...
const GameSystem = ({ game, gameHistory, setGameHistory, isArchive = false }) => {
    ...
    const trackPlayer = async (finalScore, usedGuesses, hasWon) => {
        try {
            ...
            // Silently handle Firebase errors (offline, config issues, etc.)
            console.warn('Firebase tracking failed (likely offline or not configured):', error.message);
            return null;
        }
    };
    ...
    useEffect(() => {
        if (isArchive) {
            if (matchedScore) {
                // Game already played - just show the completed state without triggering game over
                ...
                // Fresh archive game - reset state
                setGameState({
                    ...initialGameState,
                    releaseDate: game.releaseDate,
                    date: game.date,
    ...
    useEffect(() => {
        if (game && game.releaseDate !== gameState.releaseDate) {
            ...
            if (existingGameEntry) {
                // Game already played - restore the completed state
                setGameState((prevState) => ({
                    ...prevState,
                    hints: {
                        publisher: true,
